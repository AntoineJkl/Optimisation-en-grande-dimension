...
t = tic;
[u_prix,~,k_prix,J_prix,~,U_prix] = DecompositionPrix(N,A,b,C,parametres,parametres_sousproblemes);
t_prix = toc(t);
...
%AFFICHAGE:
disp(' ');
FonctionAffichage = {@(x) disp(['Solution = ( ', num2str(round(x',3)),' )'])...,
                     @(J) disp(['Objectif = ', num2str(J) ])...,
                     @(it) disp(['Nombres d''iterations = ', num2str(it)])...,
                     @(t) disp(['Temps = ', num2str(t) ,' s'])};
...
CheckKKT = {@(x) disp(['- Contraintes d''inégalité ?: ',num2str(x)])...,
            @(x) disp(['- Multiplicateurs Mu positifs ?: ',num2str(x)])...,
            @(x) disp(['- Multiplicateurs Mu * Contraintes d''inégalité nul ?: ',num2str(x)])...,
            @(x) disp(['- Contraintes égalité ?: ',num2str(x)])...,
            @(x) disp(['- Gradient Lagrangien nul ?: ',num2str(x)])};

 %TEST DES CONDITIONS DE KKT A LA FIN DE CHAQUE ALGORITHME:
disp('TEST KKT A L''ISSUE DE CHAQUE ALGORITHME:');
...
disp(['Conditions KKT verifiees pour la decomposition par prix ?: ',num2str(OK_prix)]);
...
disp(['Conditions KKT verifiees pour la decomposition par quantites ?: ',num2str(OK_alloc)]);
...
disp(['Conditions KKT verifiees pour la decomposition par predictions ?: ',num2str(OK_alloc)]);
...
disp(['Conditions KKT verifiees pour la decomposition par prix ?: ',num2str(OK_prixj)]);
...
disp(['Conditions KKT verifiees pour la decomposition par prix ?: ',num2str(OK_allocj)]);
...
for i=1:n
    disp(['n = ',num2str(i),' | epsilon = ',num2str(eps_vec(i))]);
    ...
    %Decomposition par les prix:
    [~,~,k5_prix(i),~,t_prix(i),~] = DecompositionPrix(N5,A5,b5,C5,parametres,parametres_sousproblemes);
    disp(' - Decomposition Prix OK');
    %Decomposition par quantites:
    [~,~,k5_alloc(i),~,t_alloc(i),~] = DecompositionQuantites(N5,A5,b5,C5,parametres,parametres_sousproblemes);
    disp(' - Decomposition Quantites OK');
    %Decomposition par prediction:
    [~,~,k5_pred(i),~,t_pred(i),~] = DecompositionPrediction(N5,A5,b5,C5,parametres,parametres_sousproblemes);
...
title({'Evolution du nombre d''iterations','en fonction des valeurs de precision'});
...
title({'Evolution du temps d''execution','en fonction des valeurs de precision'});
legend('Décomposition par prix','Décomposition par quantités','Décomposition par prédiction');
xlabel('Précision (\epsilon)');
...
for i=1:n
    disp(['n = ',num2str(i),' | rho = ',num2str(rho_vec(i))]);
...
title({'Evolution du nombre d''iterations','en fonction des valeurs de pas'});
...
title({'Evolution du temps d''execution','en fonction des valeurs de pas'});
legend('Décomposition par prix','Décomposition par quantités');
xlabel('Pas (\rho)');
ylabel('Temps d executon (en s)');
