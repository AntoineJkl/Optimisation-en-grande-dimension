function [u,v,w,k,J,u_stock] = DecompositionQuantites(N,A,C,param)
    ...
    %Pour ajouter l'algorithme Uzawa
    addpath('..\Algorithme');
    ...
    if ismember('rho_sp',fieldnames(param)) ; rho_sp=param.rho_sp ; else rho_sp=2 ; end
    ...
    while( k <= 2 || ((norm(u - u_prec,2) > eps) && k <= kmax))
        ...
        for i = 1:N
            ...
            elseif strcmp(algo,'explicite')
                ...
                %Resolution par la methode du point interieur
        ...
        %Coordination:
        w = w + rho*(p-repmat(mean(p,1),N+1,1));
    ...
    %Test d'appartenance de u à son ensemble admissible
    for i=1:N
